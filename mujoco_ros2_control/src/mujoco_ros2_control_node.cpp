...
int main(int argc, const char **argv) {
  ...
  // @TODO: Add a parameter to specify the name of node
  ...
  mujoco_ros2_control::MujocoRos2Control control(node, mujoco_model,
                                                 mujoco_data);
  control.init();
  RCLCPP_INFO_STREAM(
      node->get_logger(),
  ...
  RCLCPP_INFO_STREAM(node->get_logger(),
                     "Mujoco rendering has been successfully initialized !");
