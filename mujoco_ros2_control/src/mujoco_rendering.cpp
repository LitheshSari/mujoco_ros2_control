...
MujocoRendering::MujocoRendering()
    : mj_model_(nullptr), mj_data_(nullptr), button_left_(false),
      button_middle_(false), button_right_(false), lastx_(0.0), lasty_(0.0) {}

void MujocoRendering::init(rclcpp::Node::SharedPtr &node, mjModel *mujoco_model,
                           mjData *mujoco_data) {
  node_ = node;
...
void MujocoRendering::update() {
  ...
  mjv_updateScene(mj_model_, mj_data_, &mjv_opt_, NULL, &mjv_cam_, mjCAT_ALL,
                  &mjv_scn_);
...
void MujocoRendering::keyboard_callback(GLFWwindow *window, int key,
                                        int scancode, int act, int mods) {
  get_instance()->keyboard_callback_impl(window, key, scancode, act, mods);
}

void MujocoRendering::mouse_button_callback(GLFWwindow *window, int button,
                                            int act, int mods) {
  get_instance()->mouse_button_callback_impl(window, button, act, mods);
}

void MujocoRendering::mouse_move_callback(GLFWwindow *window, double xpos,
                                          double ypos) {
  get_instance()->mouse_move_callback_impl(window, xpos, ypos);
}

void MujocoRendering::scroll_callback(GLFWwindow *window, double xoffset,
                                      double yoffset) {
  get_instance()->scroll_callback_impl(window, xoffset, yoffset);
}

void MujocoRendering::keyboard_callback_impl(GLFWwindow *window, int key,
                                             int scancode, int act, int mods) {
...
void MujocoRendering::mouse_button_callback_impl(GLFWwindow *window, int button,
                                                 int act, int mods) {
  // update button state
  button_left_ =
...
void MujocoRendering::mouse_move_callback_impl(GLFWwindow *window, double xpos,
                                               double ypos) {
  ...
  // move camera
  mjv_moveCamera(mj_model_, action, dx / height, dy / height, &mjv_scn_,
                 &mjv_cam_);
}
...
void MujocoRendering::scroll_callback_impl(GLFWwindow *window, double xoffset,
  ...
  mjv_moveCamera(mj_model_, mjMOUSE_ZOOM, 0, -0.05 * yoffset, &mjv_scn_,
                 &mjv_cam_);
}
} // namespace mujoco_ros2_control
