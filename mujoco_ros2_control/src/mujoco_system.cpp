...
MujocoSystem::read(const rclcpp::Time &time, const rclcpp::Duration &period) {
  // Joint states
  ...
  for (auto &data : ft_sensor_data_) {
    ...
    data.torque.data.y() =
        mj_data_->sensordata[data.torque.mj_sensor_index + 1];
    data.torque.data.z() =
        mj_data_->sensordata[data.torque.mj_sensor_index + 2];
  }
}

hardware_interface::return_type
MujocoSystem::write(const rclcpp::Time &time, const rclcpp::Duration &period) {
  // update mimic joint
  for (auto &joint_state : joint_states_) {
  ...
  for (auto &joint_state : joint_states_) {
    if (joint_state.is_position_control_enabled) {
      ...
      mj_data_->qpos[joint_state.mj_pos_adr] =
          clamp(joint_state.position_command, min_pos, max_pos);
    }

    if (joint_state.is_velocity_control_enabled) {
      // TODO: add acceleration limits
      double min_vel, max_vel;
      min_vel = joint_state.joint_limits.has_velocity_limits
                    ? -1 * joint_state.joint_limits.max_velocity
                    : -1 * std::numeric_limits<double>::max();
      min_vel = std::max(min_vel, joint_state.min_velocity_command);

      max_vel = joint_state.joint_limits.has_velocity_limits
                    ? joint_state.joint_limits.max_velocity
                    : std::numeric_limits<double>::max();
      max_vel = std::min(max_vel, joint_state.max_velocity_command);
      if (joint_state.position <= joint_state.joint_limits.min_position) {
        min_vel = 0.0;
      } else if (joint_state.position >=
                 joint_state.joint_limits.max_position) {
        max_vel = 0.0;
      }

      mj_data_->qvel[joint_state.mj_vel_adr] =
          clamp(joint_state.velocity_command, min_vel, max_vel);
    }
    ...
    if (joint_state.is_effort_control_enabled) {
      ...
      if (joint_state.position <= joint_state.joint_limits.min_position) {
        min_eff = 0.0;
        mj_data_->qvel[joint_state.mj_vel_adr] = clamp(
            joint_state.velocity, 0.0, std::numeric_limits<double>::max());
      } else if (joint_state.position >=
                 joint_state.joint_limits.max_position) {
        max_eff = 0.0;
        mj_data_->qvel[joint_state.mj_vel_adr] = clamp(
            joint_state.velocity, -1 * std::numeric_limits<double>::max(), 0.0);
      } else if (joint_state.velocity <=
                 -1 * joint_state.joint_limits.max_velocity) {
        min_eff = 0.0;
      } else if (joint_state.velocity >=
                 joint_state.joint_limits.max_velocity) {
...
bool MujocoSystem::init_sim(
  ...
  logger_ =
      rclcpp::get_logger(node_->get_name() + std::string("mujoco_system"));
...
void MujocoSystem::register_joints(
  ...
  for (size_t joint_index = 0; joint_index < hardware_info.joints.size();
       joint_index++) {
    auto joint = hardware_info.joints.at(joint_index);
    int mujoco_joint_id =
        mj_name2id(mj_model_, mjtObj::mjOBJ_JOINT, joint.name.c_str());
    ...
    get_joint_limits(urdf_model.getJoint(last_joint_state.name),
                     last_joint_state.joint_limits);
    ...
    if (joint.parameters.find("mimic") != joint.parameters.end()) {
      ...
      if (param_it != joint.parameters.end()) {
        last_joint_state.mimic_multiplier =
            std::stod(joint.parameters.at("multiplier"));
      } else {
    ...
    auto get_initial_value =
        [this](const hardware_interface::InterfaceInfo &interface_info) {
    ...
    for (const auto &state_if : joint.state_interfaces) {
      if (state_if.name == hardware_interface::HW_IF_POSITION) {
        state_interfaces_.emplace_back(joint.name,
    ...
    for (const auto &command_if : joint.command_interfaces) {
      if (command_if.name == hardware_interface::HW_IF_POSITION) {
        command_interfaces_.emplace_back(joint.name,
                                         hardware_interface::HW_IF_POSITION,
